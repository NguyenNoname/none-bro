...
spawn(function() 
    ...
    wait(.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>Thì NguyenNoName đã trở lại!!!!!<Color=/>"):Display()
    wait(.14)
...
local Window = OrionLib:MakeWindow({Name = "NguyenNoName", HidePremium = false, IntroText = "NightHub Library", SaveConfig = true, ConfigFolder = "NguyenNoName"})
...
OrionLib:MakeNotification({
	Name = "NguyenNoName",
	Content = "NguyenNoName Not Skidder, Please don't says",
	Image = "",
	Time = 25
})
...
W:AddLabel("[+] NguyenNoName is back!")
...
pcall(function()
    while wait() do
        if _G.AutoEliteHunter then
            if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                OrionLib:MakeNotification({
                    Name = "NguyenNoName(Noitification)",
